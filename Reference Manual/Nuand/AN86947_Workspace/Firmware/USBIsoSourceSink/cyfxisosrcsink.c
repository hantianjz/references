        ...
        void)
{
    ...
    /* Set the DMA channel's size and count parameters based on the constants defined
     * in the cyfxisosrcsink.h header file.
     */
    dmaCfg.size      = CY_FX_ISOSRCSINK_DMA_BUF_SIZE;
    ...
    apiRetStatus = CyU3PDmaChannelCreate (&glChHandleIsoSink,
    ...
    for (index = 0; index < CY_FX_ISOSRCSINK_DMA_BUF_COUNT; index++)
    {
        ...
        CyU3PMemSet (buf_p.buffer, CY_FX_ISOSRCSINK_PATTERN, buf_p.size);
...

    {
                ...
                && (wValue == CY_U3P_USBX_FS_EP_HALT))
        {
            if (glIsApplnActive)
            {
                ...
                {
                    CyU3PDmaChannelReset (&glChHandleIsoSink);
                    CyU3PUsbFlushEp(CY_FX_EP_PRODUCER);
                    CyU3PUsbResetEp (CY_FX_EP_PRODUCER);
                    CyU3PDmaChannelSetXfer (&glChHandleIsoSink, CY_FX_ISOSRCSINK_DMA_TX_SIZE);
                ...
                {
                    CyU3PDmaChannelReset (&glChHandleIsoSrc);
                    CyU3PUsbFlushEp(CY_FX_EP_CONSUMER);
                    CyU3PUsbResetEp (CY_FX_EP_CONSUMER);
                    CyU3PDmaChannelSetXfer (&glChHandleIsoSrc, CY_FX_ISOSRCSINK_DMA_TX_SIZE);
                    CyU3PUsbStall (wIndex, CyFalse, CyTrue);
                    isHandled = CyTrue;
                }
            }
        ...
        void)
{
    ...
    retThrdCreate = CyU3PThreadCreate (&isoSrcSinkAppThread,           /* ISO loop App Thread structure */
                          "21:ISO_loop_MANUAL_IN_OUT",            /* Thread ID and Thread name */
...
main (void)
{
    ...
    /* Initialize the caches. Enable Instruction and Disable Data Caches. */
